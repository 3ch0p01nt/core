...
async def test_external_toggle_resets_min_cycle(
    ...
    fake_changed = datetime.datetime.now(  # pylint: disable=home-assistant-enforce-now
        dt_util.UTC
    )
...
async def test_temp_change_ac_trigger_on_long_enough_3(hass: HomeAssistant) -> None:
    ...
    await hass.async_block_till_done()
    await common.async_set_temperature(hass, 25)
    test_time = datetime.datetime.now(  # pylint: disable=home-assistant-enforce-now
        dt_util.UTC
    )
...
async def test_cycle_cooldown_schedules_restart_after_cooldown(
    ...
    hass.config.temperature_unit = UnitOfTemperature.CELSIUS
    now = datetime.datetime.now(  # pylint: disable=home-assistant-enforce-now
        dt_util.UTC
    )
    freezer.move_to(now)
